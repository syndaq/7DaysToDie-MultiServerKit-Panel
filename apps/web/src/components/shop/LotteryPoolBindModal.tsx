...
interface LotteryPoolBindModalProps {
  pool: LotteryPool;
  onClose: () => void;
...
export function LotteryPoolBindModal({ pool, onClose, onSaved }: LotteryPoolBindModalProps) {
  ...
  const saveMutation = useMutation({
    mutationFn: () =>
      api.updateLotteryPoolBindings(pool.id, {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 flex max-h-[85vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-solid)] shadow-2xl">
        <div className="border-b border-[var(--color-border)] px-5 py-4">
          <h2 className="text-lg font-semibold">Bind rewards — {pool.name}</h2>
          <p className="mt-1 text-sm text-[var(--color-muted)]">
            Define item drops and console commands for this lottery pool.
          </p>
        </div>
            ...
            <div className="space-y-4">
              ...
              <ul className="space-y-2">
                {items.map((item, index) => (
                  <li key={index} className="flex items-center justify-between rounded-xl border border-[var(--color-border)] px-3 py-2 text-sm">
            ...
            <div className="space-y-4">
              ...
              <ul className="space-y-2">
                {commands.map((command, index) => (
                  <li key={index} className="flex items-center justify-between rounded-xl border border-[var(--color-border)] px-3 py-2 text-sm">
                    <code className="truncate">{command.command}</code>
