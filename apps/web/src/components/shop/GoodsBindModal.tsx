...
export function GoodsBindModal({ product, onClose, onSaved }: GoodsBindModalProps) {
  ...
  const [commandDraft, setCommandDraft] = useState<CommandDraft>({
    command: '',
    inMainThread: false,
    description: '',
  });
  ...
  const saveMutation = useMutation({
    mutationFn: () =>
      api.updateShopProductBindings(product.id, {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 flex max-h-[85vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-solid)] shadow-2xl">
        <div className="border-b border-[var(--color-border)] px-5 py-4">
          <h2 className="text-lg font-semibold">Bind rewards — {product.name}</h2>
          <p className="mt-1 text-sm text-[var(--color-muted)]">
            Define item drops and console commands for this product. Bindings are stored cluster-wide and synced to all servers.
          </p>
        </div>
          ...
          {tab === 'items' ? (
            <div className="space-y-4">
              <div className="grid gap-3 lg:grid-cols-2">
                ...
                <Input label="Quality" type="number" value={itemDraft.quality} onChange={(e) => setItemDraft({ ...itemDraft, quality: Number(e.target.value) })} />
                <Input label="Durability" type="number" value={itemDraft.durability} onChange={(e) => setItemDraft({ ...itemDraft, durability: Number(e.target.value) })} />
                <Input label="Description" className="lg:col-span-2" value={itemDraft.description} onChange={(e) => setItemDraft({ ...itemDraft, description: e.target.value })} />
              </div>
              <Button
                variant="secondary"
                onClick={() => {
              ...
              <ul className="space-y-2">
                {items.map((item, index) => (
                  <li key={`${item.itemName}-${index}`} className="flex items-center justify-between rounded-xl border border-[var(--color-border)] px-4 py-3">
                    <div>
                      <p className="font-medium">{item.itemName} ×{item.count}</p>
                      <p className="text-xs text-[var(--color-muted)]">Quality {item.quality} · Durability {item.durability}</p>
                    </div>
                    <Button variant="ghost" size="sm" className="!text-[var(--color-danger)]" onClick={() => setItems((prev) => prev.filter((_, i) => i !== index))}>
                      Remove
                    </Button>
                  </li>
                ))}
            ...
            <div className="space-y-4">
              ...
              <Input label="Description" value={commandDraft.description} onChange={(e) => setCommandDraft({ ...commandDraft, description: e.target.value })} />
              ...
              <ul className="space-y-2">
                {commands.map((command, index) => (
                  <li key={`${command.command}-${index}`} className="flex items-center justify-between rounded-xl border border-[var(--color-border)] px-4 py-3">
                    <div>
                      <p className="font-mono text-sm">{command.command}</p>
                      {command.inMainThread && <p className="text-xs text-[var(--color-muted)]">Main thread</p>}
                    </div>
                    <Button variant="ghost" size="sm" className="!text-[var(--color-danger)]" onClick={() => setCommands((prev) => prev.filter((_, i) => i !== index))}>
                      Remove
                    </Button>
                  </li>
                ))}
