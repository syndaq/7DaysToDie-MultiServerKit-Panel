...
export function CdKeyBindModal({ cdKey, onClose, onSaved }: CdKeyBindModalProps) {
  ...
  useEffect(() => {
    ...
    setCommands(cdKey.commands.map((command) => ({
  ...
  const saveMutation = useMutation({
    mutationFn: () =>
      api.updateCdKeyBindings(cdKey.id, {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 flex max-h-[85vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-solid)] shadow-2xl">
        <div className="border-b border-[var(--color-border)] px-5 py-4">
          <h2 className="text-lg font-semibold">Bind rewards — {cdKey.code}</h2>
          <p className="mt-1 text-sm text-[var(--color-muted)]">Items and commands granted when this key is redeemed.</p>
        </div>
